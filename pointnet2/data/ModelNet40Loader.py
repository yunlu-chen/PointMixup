...
class ModelNet40Cls(data.Dataset):
    def __init__(self, num_points, transforms=None, train=True, download=True, keeprate=1):
        super().__init__()
        ...
        for f in self.files:
            points, labels = _load_data_file(os.path.join(BASE_DIR, f))#<class 'tuple'>: (2048, 2048, 3), <class 'tuple'>: (2048, 1)
        ...
        # （9840，2048, 3）
        self.labels = np.concatenate(label_list, 0)  # (9840, 1)
    ...
    def __getitem__(self, idx):
        ...
        current_points = self.points[idx, pt_idxs].copy()  # <class 'tuple'>: (1024, 3)

        if self.transforms is not None:
            current_points = self.transforms(current_points)

        label = torch.from_numpy(self.labels[idx]).type(torch.LongTensor)  # torch.Size([1])

        return current_points, label  # torch.Size([1024, 3]) torch.Size([1])
